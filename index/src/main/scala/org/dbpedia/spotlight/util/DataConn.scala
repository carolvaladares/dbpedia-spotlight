...
object DataConn {
  ...
  /**
   * Creates a Jena dataset populating both Labels (as default Model) and  a specif named Model 
   * (another dataset, for instace) 
   */
  def createTDB(
    labelData: String, 
    labelFormat: String, 
  ...
  /**
   * Creates a Jena Dataset, and adds the InputStream file  as the default Model
   */
  ...
  /**
   * Adds a new NamedModel into a TDB. 
   * It might any dataset.
   */
  ...
  */
  ...
  /** 
   *  Query data from the Default Model .
   * For  Context Extraction, it must be the Labels DBpedia dataset.
   */
